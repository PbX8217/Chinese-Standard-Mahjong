...
class dataset(Dataset):
    ...
    def __init__(self, state: str):
        ...
        localpath = "stdmahjong/data/pre/all/drop_%s" % state
        self.inputs, self.masks, self.labels = [], [], []
        choose = sample(dataset.idxs, 5)
        print("pack", end=" ")
        for k in choose:
            print(k, end=" ")
...
def training(train):
    ...
    for i, sample in enumerate(train, 1):
        ...
        output = model(inputs)
        pred = pred = torch.argmax(softmax(output, 1) * masks, 1).detach()
...
if __name__ == "__main__":
    file = open("stdmahjong/log/drop.txt", "a+")
    dst = "stdmahjong/model/drop.pth"
    model = discard_model().cuda()
    ...
    if trained < epoch:
        ...
        for k in range(trained + 1, epoch + 1):
            ...
            print("drop model saved after %d epoch" % k)
